"""Gateway must-deliver notes on the current user message (prompt-tail freeze).

The gateway relocates per-turn volatile facts OUT of the ephemeral system
prompt — routing directives, voice-channel changes, the auto-reset note, the
first-contact intro — and stages them on ``agent._gateway_turn_context_notes``.
...
DIRECTIVE = '[Routing directive: label=RUNTIME_OVERRIDE target=codex-nekos/gpt-5.6-sol/max strictness=auto_reconsiderable confidence=unknown source=pre_gateway_dispatch reason="codex-lb PR/issue review route"]'
...
class TestConsumeIsOneShot:
    def test_consume_clears_the_attribute(self):
        agent = _FakeAgent()
        agent._gateway_turn_context_notes = DIRECTIVE
        assert consume_gateway_turn_context_notes(agent) == DIRECTIVE
...
class TestStringContentSidecarDelivery:
    def test_notes_ride_the_api_content_sidecar(self):
        ...
        bytes (patch-#17 replay keeps them byte-stable in history)."""
        agent = _FakeAgent()
        agent._gateway_turn_context_notes = DIRECTIVE
...
class TestMultimodalFallback:
    def test_notes_appended_as_text_part_on_list_content(self):
        ...
        msg = ctx.messages[ctx.current_turn_user_idx]
        assert msg["content"][-1] == {"type": "text", "text": DIRECTIVE}
