...
``_create_request_openai_client`` used to build a fresh ``openai.OpenAI``
client (new httpx pool, TCP+TLS handshake, ~19-35ms) for every LLM call and
``_close_request_openai_client`` tore it down after. The agent now caches ONE
reusable wire client keyed by the effective client kwargs:
