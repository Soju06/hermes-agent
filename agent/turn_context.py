...
def build_turn_context(
    ...
    *,
    ...
    # Add user message.  A resumed turn appends NOTHING — the transcript tail
    # (already normalized by ``agent.turn_resume.prepare_resume_history``) IS
    # the turn in progress.  ``_persist_user_message_idx`` stays None so
    ...
    with turn_trace.span("prologue.persist_early", trace=_tt):
        # Retry the row create if the earlier attempt failed transiently.
        agent._ensure_db_session()
        try:
